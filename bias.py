...
def app():
    ...
    st.markdown("<p class='font'> - Encodage : l’utilisation d’images en RGB diminue drastiquement l’impact du biais    </p>", unsafe_allow_html=True)
    st.markdown("<p class='font'> - Dimensions   de   l’image :  augmenter   les   dimensions   des   images diminue l’impact du biais </p>", unsafe_allow_html=True)
    st.markdown("<p class='font'> - Prétraitement : l’utilisation de la fonction de prétraitement spécifique au modèle  diminue  l’impact du biais par rapport à un simple rescale par 1/255 </p>", unsafe_allow_html=True)
    st.markdown("<p class='font'> - Fine-tuning :  l’effet dépend du modèle et des classes (ResNet50 et DenseNet201 : diminue l’impact sur la classe Normal et l’augmente sur la classe COVID-19 ; légère augmentation globale pour InceptionV3) </p>", unsafe_allow_html=True)
